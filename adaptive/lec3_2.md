...
---
# 相互排除
最も簡単なアクセス制御方法は、各資源の管理をするプロセス（サーバ）を用意すること。それ以外のプロセス（クライアント）はサーバにアクセス要求を送り、サーバは適切なクライアントを選び、アクセスを許可する。これは段階的退化性を満たさない。
...
---
# 可用度
**可用度**：$R$ へのアクセスが可能である確率

$p$：プロセスが停止故障を起こす確率
サーバ／クライアント方式：$1-p$
MAJ-CONS (改良前)：
$$
\sum_{k=M}^n \binom{n}{k} p^{n-k} (1-p)^k
$$

$p\le0.5$ なら MAJ-CONS の可用度は $1-p$ 以上
